...
public:
    ...
    Complex& operator+=(Complex& num){
        ...
        this->imaginary += num.getImaginaryPart();
        return *this;
    }
};
...
void DFS(const Node* uzol, list<char> *finalList) {
    if (uzol) {
        if (isupper(uzol->value)) {
            finalList->push_back(uzol->value);
        }
        if (!(uzol->children.empty())){
            list<Node*>::const_reverse_iterator it;
            for (it = uzol->children.rbegin(); it != uzol->children.rend(); it++){
                DFS(*it, finalList);
            }
        }
    }
...
list<char> depthFirstSearchUpperCases(const Tree *tree) {
    list<char> allCaps;
    DFS(tree->root, &allCaps);
    return allCaps; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
City *getCity(Planet *p, const string &name) {
    list<City>::iterator it = find_if(p->cities.begin(), p->cities.end(), [&name](City& c) {
        return c.name == name;
    });
    if (it != p->cities.end()) {
        return &(*it);
    }
    throw CityNotExistsException(name);
}


void bfs1(Planet *p, const string &text, list<string> *listt) {

    City* c = getCity(p, text);
    if(!c) return;
    queue<City*> q;
    ...
    visited[c] = true;
    listt->push_back(c->name);
    q.push(c);
    while(!q.empty()){
        for(RoadTo& r : q.front()->roads){
            if(!visited[r.city]){
                visited[r.city] = true;
                listt->push_back(r.city->name);
                q.push(r.city);
            }
        }
        q.pop();
...
list<string> breadthFirstSearchReachable(Planet * planet, const string & startCity) {
    list<string> listt;
    bfs1(planet,startCity,&listt);
    return listt; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
map<string, unsigned> dijkstra(Planet * planet, const string & startCity) {
    getCity(planet, startCity);
    map<string, unsigned> mappp;
    mappp.insert(make_pair(planet->cities.begin()->name, planet->cities.begin()->roads.begin()->length));
    return mappp; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
