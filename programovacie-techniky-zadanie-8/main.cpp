...
double average(const initializer_list<int> & data) {
    ...
    return ((std::accumulate(data.begin(), data.end(), 0.0)) / data.size()); // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<int> add(const list<int> & data1, const list<int> & data2) noexcept {
    ...
    list<int> finList(data1.size());
    std::transform (data1.cbegin(), data1.cend(), data2.cbegin(), finList.begin(), std::plus<int>());
    return finList; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
