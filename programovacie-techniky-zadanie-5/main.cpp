...
void siftDown(int data[], const size_t topIndex, const size_t length) {
    size_t indexLeftChild = 2*topIndex+1;
    size_t indexRightChild = 2*topIndex+2;
    size_t indexToSwap = topIndex;

    if (indexLeftChild >= length){
        return;
    }
    if (data[indexLeftChild] < data[topIndex]){
        indexToSwap = indexLeftChild;
    }
    if (indexRightChild < length && data[indexRightChild] < data[indexToSwap]){
        indexToSwap = indexRightChild;
    }

    if (indexToSwap != topIndex){
        swap(&data[topIndex], &data[indexToSwap]);
        siftDown(data, indexToSwap, length);
    }
}
...
void buildHeapSiftDown(int data[], const size_t length) {
    for (size_t index = (length/2)-1;  index != SIZE_MAX ; index--) {
        siftDown(data, index, length);
    }
}
...
int main() {
    /*
    int nums[] = {7, 2, 1, 2, 8, 5, 3, 4, 2, 2, 6};
